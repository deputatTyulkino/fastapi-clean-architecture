...
async def add_item_to_cart(
    ...
    cart_item = await _get_cart_item(db, user.id, payload.product_id)
    if cart_item:
        ...
        db.add(cart_item)
    await db.commit()
    updated_item = _get_cart_item(db, user.id, payload.product_id)
    return updated_item
...
async def update_cart_item(
    ...
    cart_item = _get_cart_item(db, user.id, product_id)
    if not cart_item:
        raise HTTPException(
    ...
    cart_item.quantity = payload.quantity
    await db.commit()
    updated_item = _get_cart_item(db, user.id, product_id)
    return updated_item
